...
CREATE TABLE `doc` (
  ...
  `upload_user` varchar(255) NOT NULL COMMENT '上传者用户名',
  `date` DATETIME NOT NULL COMMENT '上传时间',
  `downloads` int(11) DEFAULT 0 COMMENT '下载次数',
  `category_id` int(11) NOT NULL COMMENT '文件所属类别id',
  `category` varchar(255) NOT NULL COMMENT '文件所属类别名称',
  `type` varchar(255) NOT NULL COMMENT '文件类型',
  `permission` varchar(500) COMMENT '权限列表',
